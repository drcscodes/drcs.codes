---
template: shared
title: AI Programming Project - Minimax
---

# AI Programming Project: Minimax
...
The code base has not changed much from the previous project, but please start with a fresh installation, rather than intermingling files from previous projects.
...
See the autograder tutorial in the [Search Project](../search/search.html) for more information about using the autograder.

The code for this project contains the following files, available in [minimax.zip](minimax.zip).
...
**Files to Edit and Submit**: You will fill in portions of `multiAgents.py` and submit it for this assignment.  Please do not change the other files in this distribution.

**Academic Dishonesty**: We will be checking your code against other submissions in the class for logical redundancy. If you copy someone else's code and submit it with minor changes, we will know. These cheat detectors are quite hard to fool, so please don't try. We trust you all to submit your own work only; please don't let us down. If you do, we will pursue the strongest consequences available to us.

**Topics needed for this project**:

- Adversarial Search lecture notes/slides, AIMA 5 (6 in Global Edition)
...
> Note: You may find it useful to view the internal contents of various objects for debugging. You can do this by printing the objects' string representations. For example, you can print `newGhostStates` with `print(f"{newGhostStates=}")`.
...
![Minimax tree with depth 2](minimax_depth.png){width="90%"}
...
Once your algorithm is working on small trees, you can observe its success in Pacman. Random ghosts are of course not optimal minimax agents, and so modeling them with minimax search may not be appropriate. E`xpectimaxAgent` will no longer take the min over all ghost actions, but the expectation according to your agent's model of how the ghosts act. To simplify your code, assume you will only be running against an adversary which chooses amongst their `getLegalActions` uniformly at random.
...
You should find that your `ExpectimaxAgent` wins about half the time, while your `jAlphaBetaAgent` always loses. Make sure you understand why the behavior here differs from the minimax case.
...
Submit your modified `multiAgents.py` to the assignment on D2L.
