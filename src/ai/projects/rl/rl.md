---
template: shared
title: AI Programming Project - Reinforcement Learning
---

# AI Programming Project: Reinforcement Learning
...
The code for this project contains the following files, available as a zip archive: [`rl.zip`](rl.zip)
...
- MDPs: [Markov Decision Processes slides](@root/rl/slides/mdps.pdf), AIMA 17.1, RLAI 3
- DP: [Dynamic Programming slides](@root/rl/slides/dp.pdf), AIMA 17.2, RLAI
- RL: [Temporal-Difference Learning slides](@root/rl/slides/td-learning.pdf), AIMA 22, RLAI 6
...
$$
V_{k+1}(s) \leftarrow R(s) + \max_a \sum_{s'} T(s, a, s') V_k(s')
$$
...
Note: A policy synthesized from values of depth $k$ (which reflect the next $k$ rewards) will actually reflect the next $k+1$ (i.e. you return $\pi_{k+1}$).  Similarly, the Q-values will also reflect one more reward than the values (i.e. you return $Q_{k+1}$).
...
![Bridge crossing diagram](Bridge_Crossing_Diagram.png)
...
![Paths in gridworld](value_2_paths.png)
...
![Q-value diagram](q_learning.png)
...
## Question 8: Q-Learning and Pacman
...
$$
w_i \gets w_i + \alpha \cdot \text{difference} \cdot f_i (s,a)
$$
