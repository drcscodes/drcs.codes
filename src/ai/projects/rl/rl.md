...
k
k based on the values of its successor states, the successor state values used in the value update computation should be those from iteration
k
−
1
k−1 (even if some of the successor states had already been updated in iteration
k
k). The difference is discussed in Sutton & Barto in Chapter 4.1 on page 91.

Note: A policy synthesized from values of depth
k
k (which reflect the next
k
k rewards) will actually reflect the next
k
+
1
k+1 rewards (i.e. you return
π
k
+
1
