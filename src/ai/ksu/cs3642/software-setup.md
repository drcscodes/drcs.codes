...
These instructions are for installing and configuring the software used in this course.  I use and recommend the Python.org distribution of Python, but if you already have another distribution, such as Anaconda, that will work fine as long as it includes Python >= 3.6, `pip`, and `venv`.
...
3. (Optional) Install and configure [Git](/shared/git).
...
2. Install [Homebrew](https://brew.sh/) and use the `brew` command to install Python, `brew install python3`.
...
> See detailed instructions in [Microsofts docs](https://docs.microsoft.com/en-us/windows/wsl/install-win10#simplified-installation-for-windows-insiders).  Note that Windows 10 with all updates from 2021-07-29 and later should support the simplified install.
