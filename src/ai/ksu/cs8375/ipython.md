...
All functions should have [doctests](https://docs.python.org/3/library/doctest.html).  But doctests must be in in Python REPL format.  iPython has you covered.
...
Now you can add that to  your function definition.
...
- These commands work with automagic (list them  yourself iwth `%lsmagic`):
...
1.41 ms ± 15.1 μs per loop (mean ± std. dev. of 7 runs, 1,000 loops each)

In [37]: %timeit _ = nparray * 2
620 μs ± 17.5 μs per loop (mean ± std. dev. of 7 runs, 1,000 loops each)
```

Numpy is a little faster than native Python.  (To be fair, Python now has an efficient [array type](https://docs.python.org/3/library/array.html) but by the time it was added to the Python standard library, everyone was already using and depending on Numpy.)
...
## Saving and Loading iPython Sessions
