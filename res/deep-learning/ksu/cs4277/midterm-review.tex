...
\vspace{1.5in}
  ...
  \ifprintanswers
  \begin{solution}
    The global minimum is the point at which the function's value is minimal.  Local minima are points at which the function's value is lower than it's neighbors, but not as low as the global minimum.
  \end{solution}
  \else
  \vspace{1.5in}
  \fi
  ...
  \begin{solution}
    ...
    \begin{align*}
      \bm{m}_{t+1}   &\leftarrow \beta \bm{m}_t + (1-\beta) \sum_{i \in \mathcal{B}} \frac{\partial \ell_i(\bm{\phi})}{\partial \bm{\phi}}\\
      \bm{\phi_{t+1}} &\leftarrow \bm{\phi}_t - \alpha \bm{m}_{t+1}
    \end{align*}
  ...
  \ifprintanswers
  \begin{solution}
    Adam stands for {\it adaptive moment estimation}.  The purpose of Adam is to normalize the gradients so that we move a fixed distance (governed by the learning rate) in each direction.  Without this normalization, updates are greater for parameters with the steepest gradients.  Because the magnitude of updates is different for each parameter it is difficult to choose a learning rate that that makes good progress in each direction and is stable.
  \end{solution}
  \else
  \vspace{1.5in}
  \fi
  ...
  \ifprintanswers
  \begin{solution}
    The second derivatives of the loss function with respect to the model parameters.
  \end{solution}
  \else
  \vspace{1.5in}
  \fi
...
\question Describe the efficiency of the backpropagation algorithm with respect to time and memory.
  ...
  \ifprintanswers
  \begin{solution}
    Backpropagation is time efficient but requires a great deal of storage to store intermediate values.
\end{solution}
\else
\vspace{1.5in}
\fi

\question How are the weights of a network typically initialized?
...
\question Describe the vanishing gradients problem and what initialization mistake leads to it.
  ...
  \ifprintanswers
  \begin{solution}
    If the variance $\sigma^2$ of the Gaussian used to initialize the parameters of the network is too small, then the weights may become very low, underflowing floating-point representation capability.
  \end{solution}
\else
\vspace{1.5in}
\fi
...
\question Describe the exploding gradients problem and what initialization mistake leads to it.
  ...
  \ifprintanswers
  \begin{solution}
    If the variance $\sigma^2$ of the Gaussian used to initialize the parameters of the network is too large, then the weights may become very large and training will become unstable.
  \end{solution}
\else
\vspace{1.5in}
\fi
...
\question What initialization method avoids the vanishing gradients and exploding gradients problem?
  ...
  \ifprintanswers
  \begin{solution}
    {\it He initialization}: initialize a given layer with:
    \[
