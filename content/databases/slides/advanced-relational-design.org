...
1. Dependency preservation (desired), and
2. Non-additive (lossless) joins (required).
...
The non-additive join test finds the right attribute to connect the two schemas, the attribute on which you would join the decomposed schemas to recover the original schema.\\

Remember how to test if $X \rightarrow Y$ is in $F^+$?

- $Y$ is in $X^+$ under $F$. -- use the attribute closure algorithm (Algorithm 15.1).
